...
const iveInstances = iveMembers.map(
  (member) => new FemaleIdol(member.name, member.year)
);
const btsInstances = btsMembers.map(
  (member) => new MaleIdol(member.name, member.year)
);
console.log(iveInstances);
console.log(btsInstances);

console.log("------------------------");

const ive = new IdolGroup("아이브", iveInstances);
const bts = new IdolGroup("BTS", btsInstances);
console.log(ive);
console.log(bts);
...
console.log(korea.idolGroups[0].members[0].dance());
